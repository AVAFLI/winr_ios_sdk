    ...
    private func load() async {
        do {
            ...
            if backendClaimedToday == false, hasEmailConsent,
               let day = backendStreakDay, day >= 2,
    ...
    func claimDailyEntries(auto: Bool = false) {
        ...
        Task {
            do {
                ...
                // - Day 2+: the celebration already played at mount off the
                //   PREDICTED grant — reconcile the staged numbers with the real
                //   response. In the normal case they're identical, so nothing
                //   visibly changes; a mismatch silently corrects the readouts.
                if auto {
