...
///
/// This is the remote-image sibling of `WINRV2GifAsset.prewarm`: the SDK learns
...
let winrV2ImageFadeDuration: Double = 0.2
...
/// error.
struct WINRV2RemoteImage<Fallback: View>: View {
    let url: String
    ...
    init(
        ...
        placeholderColor: Color = WINRV2Color.deepCharcoal,
        ...
        let warm = WINRV2ImageWarmer.cached(url)
    ...
    private func loadIfNeeded() {
        ...
        WINRV2ImageWarmer.load(url) { loaded in
            guard let loaded else {
                failed = true
                return
            }
            image = loaded
            withAnimation(.easeOut(duration: winrV2ImageFadeDuration)) { opacity = 1 }
        }
    }
}
